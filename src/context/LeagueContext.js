...
export const LeagueProvider = function ({ children }) {
  ...
  const championName = async (name) => {
    const getData = await fetch(
      `http://ddragon.leagueoflegends.com/cdn/12.16.1/data/en_US/champion/${name}.json`
    );
  ...
  const championImage = (name) => {
    const image = `http://ddragon.leagueoflegends.com/cdn/12.16.1/img/champion/${name}.png`;
  ...
  const backgroundImage = (name, num = "0") => {
    const image = `http://ddragon.leagueoflegends.com/cdn/img/champion/splash/${name}_${num}.jpg`;
